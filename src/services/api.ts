...
export const aiApi = {
  ...
  generateBizPlan: (formData: BizPlanForm) =>
    post<{ text: string }>(`${API_BASE}/generate-biz-plan`, formData as unknown as Record<string, unknown>).then((d) => d.text),

  generateWebsite: (description: string) =>
    post<{ text: string }>(`${API_BASE}/generate-website`, { description }).then((d) => d.text),
  ...
  marketAnalysis: (businessIdea: string) =>
    post<{ text: string }>(`${API_BASE}/market-analysis`, { businessIdea }).then((d) => d.text),

  generateAdImages: (description: string, platform: string, language?: string) =>
    post<{ platform: string; images: unknown[] }>(`${API_BASE}/generate-ad-images`, {
      description,
      platform,
      ...(language && { language }),
    }),
...
export const botApi = {
  ...
  uploadKnowledgeFile: async (userId: string, file: File) => {
    ...
    return request<{ chunksAdded: number; fileName: string }>(`${BOT_API}/knowledge/${userId}`, {
      method: "POST",
      body: formData,
    });
  },
  ...
  deleteKnowledge: (userId: string, chunkId: string) =>
    request<void>(`${BOT_API}/knowledge/${userId}/${chunkId}`, { method: "DELETE" }),
...
export const authApi = {
  login: (email: string, password: string) =>
    post<{ token: string; user: Record<string, unknown> }>(`${AUTH_API}/login`, { email, password }),

  register: (name: string, email: string, password: string) =>
    post<{ token: string; user: Record<string, unknown> }>(`${AUTH_API}/register`, { name, email, password }),

  sendOtp: (phone: string) =>
    post<{ message: string }>(`${AUTH_API}/send-otp`, { phone }),

  verifyOtp: (phone: string, code: string) =>
    post<{ token: string; user: Record<string, unknown> }>(`${AUTH_API}/verify-otp`, { phone, code }),
};
...
export const contentApi = {
  ...
  approve: (planId: string, telegramChannelId: string, scheduledPosts: unknown[]) =>
